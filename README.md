...
盈时宝的数据**自动保存在你的电脑里**，不需要注册任何账号，也无需联网：

- **打包版**：`~/Documents/盈时宝/data.db`
- **开发版**：项目目录下的 `data.db`

数据就是本地一个 SQLite 文件，备份直接复制这个文件就行。换电脑时拷贝过去就能接着用。

> 手机是通过 WiFi 远程访问电脑上的页面，不是"云同步"——电脑关了手机就看不了。
